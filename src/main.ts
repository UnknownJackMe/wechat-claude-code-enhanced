    ...
    const queryOptions: QueryOptions = {
      ...
      systemPrompt: [
        [
          ...
          '',
          '【重要：你运行在一次性进程中】',
          '你这一轮对话运行在一个"问完即退"的一次性进程里：你输出完最终回复后，进程立即结束。因此你【绝对不能】发起任何"先回复、再后台等待、稍后异步通知"的操作——那些后台任务会在你回复后随进程一起被终止，用户永远收不到结果。具体禁止：',
          '- 禁止使用任何 run_in_background / 后台模式（包括后台 Bash、后台 Task）。',
          '- 禁止启动 Monitor、watch、轮询等"挂起等待外部事件再回来"的机制。',
          '- 委派 codex 时，必须用前台同步等待（--wait，前台阻塞直到跑完），禁止 --background。',
          '- 委派 sub-agent（Task 工具）时直接用即可——它本来就是同步阻塞的，会在本轮内跑完返回，这是允许且推荐的。',
          '一句话：所有委派和工具调用都必须在你这一轮回复之前【同步跑完】，把最终结果直接写进回复里。需要长时间运行的活，就同步等它跑完再回复，哪怕等久一点也没关系。',
          '',
          '如果用户需要文件，直接在回复里写出文件的本地路径即可，系统会识别并提示用户用 /send-me 拉取该文件。微信支持 Markdown 渲染，请在回复中合理使用 **粗体**、列表（1. 或 -）、`---` 分割线来让内容更清晰，但不要过度使用，保持简洁。',
        ].join('\n'),
        config.systemPrompt,
      ].filter(Boolean).join('\n'),
      abortController,
